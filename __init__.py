...
# Try to add XTTS path to site-packages, but don't fail if we can't
pth_file_created = False
for site_packages_root in site_packages_roots:
    if os.path.exists(site_packages_root):
        try:
            ...
            pth_file_created = True
            break
        except PermissionError:
            print(f"Warning: Could not write XTTS.pth to {site_packages_root} - continuing without it")
            continue
...
if pth_file_created:
    ...
    print("Note: XTTS.pth file could not be created due to permissions, but the custom node should still work")
